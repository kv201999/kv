...
class req {
	...
	*/
	...
	function get_otcbuy() {
		...
		$otcbuyprice=$result->data[0]->price;
		return $otcbuyprice;
	}
	function get_otcsell() {
		$this->api_method ="";
		$this->req_method = 'GET';
		...
		$result = json_decode($this->curl($url));
		$otcsellprice=$result->data[0]->price;
		return $otcsellprice;
	}
	/**
	* 公共类API
	...
	*/
	...
	function get_address() {
		$this->api_method = "/v2/account/deposit/address";
		$this->req_method = 'GET';
		$postdata = [
			'currency' => "usdt"
		];
